    ...
    public void onMatchEnd(MatchEndEvent event) {
        ...
        if (event.getTeam() != null) {
            if (event.getTeam().size() == 1) {
                ChatUtils.getGlobalChannel().sendLocalizedMessage(new UnlocalizedChatMessage(ChatColor.DARK_PURPLE + "# # {0}" + ChatColor.DARK_PURPLE + " # #", new LocalizedChatMessage(ChatConstant.UI_MATCH_WIN, event.getTeam().get(0).getDisplayName())));
            } else {
